...
**重要前提**：这个计算忽略了激活值内存（取决于 batch size 和序列长度），所以只是一个**理论上界**。实际能训练的模型会比这个小。
...
这个 2 来自：对每个 (i, j, k) 三元组，做一次乘法（x[i][j] × w[j][k]）和一次加法（累加到 y[i][k]）。严格来说是 (2×D-1) 次操作，但当 D 很大时可以近似为 2D。
