...
11. [注意力头（Attention heads）](#11-注意力头（Attention heads）)
13. [总结](#13-总结)
...
FFN 的中间层是一个先膨胀再收缩的瓶颈结构。$d_{ff}$ 控制的就是这个瓶颈有多宽，膨胀得越宽，FFN 的表示容量越大，但参数和计算量也线性增长。
- $d_{model}$：残差流的维度，贯穿整个 Transformer 的"主干道宽度"
- $d_{ff}$：FFN 中间层的维度，残差流进入 FFN 后先被放大到的宽度

![Kaplan Feedforward Ratio Sweep](lecture3_images/page37_img01.png)

有两个相关维度，feedforward dim $d_{ff}$ 和 model dim $d_{model}$，他们之间的关系是什么？通常有 $d_{ff}$ = 4 $d_{model}$ ，但也有一些例外，例如使用了 GLU 变体。这意味着大多数 GLU 变体 $d_{ff} = 4 × 2/3 = (8/3) d_{model}$ 
